    ...
    def handle(self, *args, **options):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # We save directly to your E-commerce DB.
                product, created = Book.objects.get_or_create(
                    title=title,
                    defaults={
                        "price": price,
                        "description": f"Imported from Amazon. Original Image: {image_url}",  # noqa
                ...
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(f"🎉 Created new product: {product.title}")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING("Product already exists. Updated price.")
                    )
                    product.price = price
                    product.save()

...
class BookViewsetTests(APITestCase):
    ...
    def test_list_books(self):
        url = reverse('myApp_api:books-list')
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)

    def test_filter_books_by_title(self):
        url = reverse('myApp_api:books-list') + '?title=django'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_book(self):
        url = reverse('myApp_api:books-detail', args=[self.book1.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_book_admin(self):
        self.client.force_authenticate(user=self.admin)
        url = reverse('myApp_api:books-detail', args=[self.book2.id])
